...
class TransactionRepository {
  ...
  Future<model.Transaction?> getById(String id) async {
    final db = await _dbHelper.database;
    final result = await db.query(
      'transactions', where: 'id = ? AND is_deleted = 0', whereArgs: [id],
    );
    if (result.isEmpty) return null;
    return model.Transaction.fromMap(result.first);
  }

  Future<model.Transaction?> getByIdRaw(String id) async {
    final db = await _dbHelper.database;
    final result = await db.query(
  ...
  Future<List<model.Transaction>> getByCustomerRaw(String customerId) async {
    final db = await _dbHelper.database;
    final result = await db.query(
      'transactions',
      where: 'customer_id = ?', whereArgs: [customerId],
  ...
  Future<List<model.Transaction>> getByType(int type) async {
    ...
    final result = await db.query(
      'transactions',
      where: 'type = ? AND is_deleted = 0', whereArgs: [type],
      orderBy: 'date DESC',
    );
    return result.map((map) => model.Transaction.fromMap(map)).toList();
  }
  ...
  Future<void> upsertFromCloud(List<model.Transaction> records) async {
    ...
    for (final t in records) {
      final existingResult = await db.query(
        'transactions', where: 'id = ?', whereArgs: [t.id],
      );
      if (existingResult.isEmpty) {
        ...
        final existing = model.Transaction.fromMap(existingResult.first);
